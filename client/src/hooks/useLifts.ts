...
export function useLifts(mountainId: string | undefined) {
	...
	const createLift = async (lift: LiftCreatePayload) => {
	...
	const updateLift = async (liftId: string, updated: Partial<Lift>) => {
		if (!mountainId) return;
	...
	return { lifts, isLoading, fetchLifts, createLift, updateLift, deleteLift };
}
