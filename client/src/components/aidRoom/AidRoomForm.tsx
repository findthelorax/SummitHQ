...
const getEmptyForm = (): AidRoomInputPayload => ({
	name: '',
...
const AidRoomForm: React.FC<AidRoomFormProps> = ({ onCreated }) => {
	...
	const [form, setForm] = useState<AidRoomInputPayload>(getEmptyForm());
	...
	return (
        <form className="form-container" onSubmit={handleSubmit}>
            ...
            {!selectedMountain && (
                <div className="text-error text-center mt-2">Please select a mountain to add an aid room.</div>
            )}
        </form>
	);
};
