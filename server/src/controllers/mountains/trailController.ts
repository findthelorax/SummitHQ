...
class TrailController {
	createTrail = asyncWrapper(async (req: Request, res: Response) => {
		...
		const { areaId, ...trailData } = req.body;
		...
		if (!trailData.name || !trailData.difficulty) {
			res.status(400).json({ message: 'Trail name and difficulty are required' });
			return;
		}

		const trail = await TrailModel.create(mountainId, trailData, areaId);
	...
	getAllTrails = asyncWrapper(async (req: Request, res: Response) => {
		const trails = await TrailModel.findAll();
