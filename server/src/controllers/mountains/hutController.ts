...
class HutController {
	createHut = asyncWrapper(async (req: Request, res: Response) => {
		...
		const hut = await HutModel.create(mountainId, hutData, areaId);
    ...
    getAllHuts = asyncWrapper(async (req: Request, res: Response) => {
        const huts = await HutModel.findAll();
