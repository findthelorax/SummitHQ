...
class LodgeController {
	createLodge = asyncWrapper(async (req: Request, res: Response) => {
		...
		const { areaId, ...lodgeData } = req.body;
		...
		const lodge = await LodgeModel.create(mountainId, lodgeData, areaId);
	...
	getAllLodges = asyncWrapper(async (req: Request, res: Response) => {
		const lodges = await LodgeModel.findAll();
