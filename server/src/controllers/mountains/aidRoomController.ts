...
class AidRoomController {
	createAidRoom = asyncWrapper(async (req: Request, res: Response) => {
		...
		const { areaId, ...aidroomData } = req.body;
		...
		const result = await AidRoomModel.create(mountainId, aidroomData, areaId);
	...
	getAllAidRooms = asyncWrapper(async (req: Request, res: Response) => {
		const results = await AidRoomModel.findAll();
